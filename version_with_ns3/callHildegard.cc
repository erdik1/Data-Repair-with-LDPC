...
//void callHildegard(double delta, int N, int nOriginal, int kOriginal, int p, int numberofInitialNodes, int numberofInitialLDPCStorageNodes, std::string fileNameAlg1, std::string fileNameAlg2, Ptr<Socket> ns3TcpSocket, Address sinkAddress, NodeContainer gNbNodes, NodeContainer ueNodes, NetDeviceContainer gNbNetDev, NetDeviceContainer ueNetDev, MobilityHelper mobility, InternetStackHelper internet)
std::vector<std::vector<int>> callHildegard(int nOriginal, int kOriginal, int p, int numberofInitialNodes, int numberofInitialLDPCStorageNodes, std::string fileNameAlg1, std::string fileNameAlg2)
{
	int j, k;
...
/*	cout << "n is " << nOriginal << endl;
	...
	std::vector<std::vector<int>> H = generateH_Array_LDPC(p, j, k);
...
/*	//for (int co = 1; co < sizeof(H[0]); co++)
	...
	//M = size(G, 1);
	//limit = 10000;
	// [[maybe_unused]] int numberofRoundforBuffer = 1000; //udpPacketSize
	// //lambda = 1;
	// //a = M*lambda;
	...
	// hildegardVonBingen(H, G, N, a, b, numberofInitialNodes, numberofInitialLDPCStorageNodes, delta, rawFileName, numberofRoundforBuffer, p, j, k, fileNameAlg1, fileNameAlg2, ns3TcpSocket, sinkAddress, gNbNodes, ueNodes, gNbNetDev, ueNetDev, internet);
