...
//Struct SecondPhaseV2(std::vector<int> currentLostLDPCChunkSymbols, std::vector<int> L, std::vector<std::vector<int>> R2, std::vector<std::vector<int>> R2Reduced, struct strP strParameters, std::vector<std::vector<uint8_t>> encodedData, int currentColumnIndex, int cols, int stopState, int maxColLength, int quotient, int selectedEmptyNodeIndex, NetDeviceContainer ueNetDev, NetDeviceContainer gNbNetDev, int numberofRoundforBuffer, Ipv4InterfaceContainer ipInterfs, Ipv4InterfaceContainer staInterface, uint16_t servPort, Ptr<Node> apWifiNode, Ipv4InterfaceContainer ueIpIface, Ptr<Node> remoteHost, std::vector<string> inputFiles, std::vector<int> aliveLDPCStorageNodes, WifiHelper wifiHelper)
Struct SecondPhaseV2(std::vector<int> currentLostLDPCChunkSymbols, std::vector<int> L, std::vector<std::vector<int>> R2, std::vector<std::vector<int>> R2Reduced, struct strP strParameters, std::vector<std::vector<uint8_t>> encodedData, int currentColumnIndex, int cols, int stopState)
{
	Struct sprLocal;
	FILE *baseStationID;
	...
	while (!L.empty())
	{
		...
		if (lostHelpers.size() >= 1)
		{
			...
			// tcpAppS1.data = encodedData[failedSymbolID];
			// std::vector<uint8_t> encodedDataFromBS = encodedData[failedSymbolID];

			// tcpAppS1->SetReceiverSocket();
			// 	tcpAppS1->SetSenderSocket();
			// 	MType messageType = REQUESTREPAIRFROMBS;
		 //        tcpAppS1->StartFlowV3(messageType, encodedDataFromBS);
			// 	//tcpAppS1->StartFlow();
			// //---------------------------------------------------------
		}
		else
